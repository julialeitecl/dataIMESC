class Reg_Plan:
    ...
    def organiza(self):
        ...
        import pandas as pd
        x=(n for n in range(250))
        for f in x:
            try:
                req=rq.get(f"http://dataimesc.imesc.ma.gov.br/getData?id={f}&scope=4&from=2010&to=2021")
                req=req.json()
                requi_series=req["values"]
                ...
                mat=mat.groupby("nivel").sum(numeric_only=True)
                mat=mat.reset_index()
                mat.insert(0, "serie", f)
                mat.insert(1, "abrangencia", 5)
                mat.insert(2, "nome", "aguardando")
                #Iniciando renomeação das séries
                ...
                mat=pd.melt(mat,id_vars=["serie", "abrangencia","nome","nivel"],value_vars=[str(x) for x in range(2010,2021)])
                print(mat[["serie","abrangencia","nome","value","variable"]])
                #mat.to_excel("pib_somado_serie.xlsx", index=False)
            except:
                print(f"Tabela {f} nem rodou")
